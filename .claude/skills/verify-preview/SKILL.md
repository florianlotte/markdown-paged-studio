...
2. Run `npm run lint` first. Fix lint errors before opening the browser.
...
5. Wait for the render. The toolbar `#status` span shows `Rendu…` while Paged.js works, then `N page(s)` on success or `Erreur de rendu` on failure. Poll with `find` for the status text; give it up to 10 seconds.
...
9. Do not click `Imprimer / PDF`: it calls `window.print()`, which opens a blocking dialog that stalls the browser session. Check the export path by reading the generated HTML string instead.
